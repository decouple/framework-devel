...
class MySQLTableAlter extends MySQLTableStructure implements TableAlterInterface {
  protected Vector<string> $dropped;
  protected Vector<TableColumnInterface> $changed;
  ...
  public function execute() : bool {
    ...
    if($this->changed->count()) {
      foreach($this->changed as $column) {
        $sql .= "MODIFY COLUMN " . $this->columnDefinition($column, true) . "\n";
      }
    }
    $sql = substr($sql, 0, -2) . ';';
